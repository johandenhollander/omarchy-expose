...
ShellRoot {
    ...
    function finish() {
        console.log("PASS: real shell theme tokens, views, gradients, per-side widths, alpha, zero borders, live reload");
        Qt.quit();
    }
    ...
    Timer {
        ...
        onTriggered: {
            if (test.phase === 0) {
                ...
                require(card.outlineSpec.widths.left === 0 && card.outlineSpec.gradient.enabled, "actual window card uses full spec");
